...
public class ImageAndroid implements Image {
    ...
    public Image resizeImage(Image image, int width, int height) {
        ((ImageAndroid) image).setBitmap(Bitmap.createScaledBitmap(((ImageAndroid) image).getBitmap(), width, height, true));
        return image;
    }

    @Override
    public Image cropImage(Image image, int x1, int y1, int x2, int y2) {
        ((ImageAndroid) image).setBitmap(Bitmap.createBitmap(((ImageAndroid) image).getBitmap(), x1, y1, x2 - x1, y2 - y1));
        return image;
    }
